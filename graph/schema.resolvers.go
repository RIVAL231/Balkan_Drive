...
func (r *mutationResolver) Login(ctx context.Context, email string, password string) (*model.AuthPayload, error) {
	...
	err := r.DB.QueryRow(ctx,
		...
		email).Scan(&user.ID, &user.Username, &user.Email, &hashedPassword, &role)
	if err != nil {
...
func (r *mutationResolver) CompleteUpload(ctx context.Context, uploadToken string) (*model.File, error) {
	...
	var file model.File
	err := r.DB.QueryRow(ctx,
		`SELECT id, filename, filetype, filesize, filehash, is_public, created_at 
		 FROM files WHERE filehash = $1 ORDER BY created_at DESC LIMIT 1`,
		uploadToken,
	).Scan(&file.ID, &file.Filename, &file.Filetype, &file.Filesize, &file.Filehash, &file.IsPublic, &file.CreatedAt)
...
func (r *queryResolver) GetFile(ctx context.Context, fileID string) (*model.File, error) {
	var file model.File
	err := r.DB.QueryRow(ctx,
		"SELECT id, filename, filetype, filesize, filehash, is_public, created_at FROM files WHERE id = $1",
		fileID,
	).Scan(&file.ID, &file.Filename, &file.Filetype, &file.Filesize, &file.Filehash, &file.IsPublic, &file.CreatedAt)
...
func (r *queryResolver) ListFiles(ctx context.Context, folder *string, isPublic *bool, ownerID *string) ([]*model.File, error) {
	query := `SELECT id, filename, filetype, filesize, filehash, is_public, created_at FROM files WHERE 1=1`
	args := []interface{}{}
	argCount := 0
	...
	for rows.Next() {
		var file model.File
		err := rows.Scan(&file.ID, &file.Filename, &file.Filetype, &file.Filesize, &file.Filehash, &file.IsPublic, &file.CreatedAt)
		if err != nil {
