...
test_database() {
    ...
    if ! docker compose exec -T database pg_isready -h localhost -U admin &>/dev/null; then
        log_error "PostgreSQL is not ready"
        return 1
    fi
    
    # Test database exists and is accessible
    if ! docker compose exec -T database psql -U admin -d graphqlmvp -c "SELECT 1;" &>/dev/null; then
        log_error "Cannot connect to graphqlmvp database"
        return 1
    fi
    
    # Check essential tables exist
    local table_count=$(docker compose exec -T database psql -U admin -d graphqlmvp -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('users', 'files', 'folders', 'content', 'audit_logs');" 2>/dev/null | tr -d ' \n')
...
main() {
    ...
    wait_for_service "PostgreSQL" "docker compose exec -T database pg_isready -h localhost -U admin" 60 || exit 1
